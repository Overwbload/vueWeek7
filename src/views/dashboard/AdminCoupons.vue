<template>
  ...
  <DelCouponModal ref="delCouponModal" :temp-coupon="tempCoupon" @del-coupon="confirmDel"></DelCouponModal>
  </div>
</template>
<script>
...
import couponModal from '../../components/ConuponModal.vue'
...
export default {
  computed: {
    ...mapState(couponsStore, ['pagination', 'coupons', 'loadingStatus', 'status', 'loadingIcon', 'tempCoupon'])
  },
  components: { LoadingOverlay, PaginationComponent, couponModal, DelCouponModal },
  methods: {
    ...mapActions(couponsStore, ['getCoupon', 'openModal', 'enabled', 'confirmDel'])
  },
  mounted () {
    this.getCoupon()
    watchEffect(() => {
